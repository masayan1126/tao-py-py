...
def test_ファイルオブジェクトからファイルのurlを取得できる(setuped_xfile: XFile):
    assert (
        setuped_xfile.x_url().href()
        == "https://3.bp.blogspot.com/-X6ruoCYjbMQ/WSa8zi2McUI/AAAAAAABEhc/OtxhrQP4PYIusK-uT61_NHbxUmlEbLWgACLcB/s800/cat_mikeneko2.png"
    )


def test_ファイルオブジェクトからファイル名のみを取得できる(setuped_xfile: XFile):
    assert setuped_xfile.filename() == "cat_mikeneko2.png"


def test_ファイルオブジェクトから拡張子を取得できる(setuped_xfile: XFile):
