...
def test_文字列型以外を指定した場合は例外():
    with pytest.raises(TypeError):
        xstr = XStr(1)


def test_対象の文字列に特定の文字が含まれているかチェックできること_含まれていない場合(setuped_xstr: XStr):

    assert setuped_xstr.is_contain("hoge") == False


def test_対象の文字列に特定の文字が含まれているかチェックできること_含まれている場合(setuped_xstr: XStr):

    assert setuped_xstr.is_contain("yan") == True


def test_対象の文字列が特定の文字で始まっているかチェックできること(setuped_xstr: XStr):

    assert setuped_xstr.has_begin("ma") == True


def test_対象の文字列が特定の文字で終わっているかチェックできること(setuped_xstr: XStr):
