...
def test_OSに依存しないパス文字列からパスオブジェクトを生成できる() -> None:
    ...
    actual = XFileSystemPath(XStr("tests/x_file_system_path.py")).to_text()
...
def test_現在の作業ディレクトリを取得できる() -> None:
    ...
    actual = XFileSystemPath.cwd().to_text()
...
def test_絶対パスから相対パスへ変換できる() -> None:
    ...
    expected = str(pathlib.Path("tests/x_file_system_path.py"))
    actual = XFileSystemPath(XStr(abs_path_str)).to_relative().to_text()
...
def test_絶対パスから相対パスへ変換できる_基底パスを動的に切り替え可能() -> None:
    ...
    expected = "git"
    actual = (
        XFileSystemPath(XStr(abs_path.to_text()))
        .to_relative(base_path=XFileSystemPath.home_dir().to_text())
        .to_text()
    )
...
def test_相対パスから絶対パスへ変換できる() -> None:
    ...
    actual = XFileSystemPath(XStr(relative_path_str)).to_absolute().to_text()
...
def test_ホームディレクトリを取得できる() -> None:
    ...
    actual = XFileSystemPath.home_dir().to_text()
...
def test_パスを追加できる() -> None:
    ...
    assert sut.to_text() == str(pathlib.Path("tests/hoge/sample"))


def test_パスを連続で追加できる() -> None:
    sut = XFileSystemPath(XStr("tests")).join("hoge/sample")
    sut.join("foo")
    assert sut.to_text() == str(pathlib.Path("tests/hoge/sample/foo"))
